...
void saveUsersToFile(const char* filename) {
    ...
    for (int i = 0; i < userCount; i++) {
        fprintf(file, "%s,%s,%s,%s,%d,%d,%d,%d,%d\n",
                ...
                users[i].gender, users[i].birthDay, users[i].birthMonth,
                users[i].birthYear, users[i].isLocked);
    }
...
void loadUsersFromFile(const char* filename) {
    ...
    userCount = 0;
    while (fscanf(file, "%19[^,],%49[^,],%49[^,],%19[^,],%d,%d,%d,%d,%d\n",
                  users[userCount].id, users[userCount].name, users[userCount].email,
                  users[userCount].phone, &users[userCount].gender,
                  &users[userCount].birthDay, &users[userCount].birthMonth,
                  &users[userCount].birthYear, &users[userCount].isLocked) == 9) {
        userCount++;
    }
